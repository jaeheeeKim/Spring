...
// @WebServlet오류는 Build Path>Configure Build Path>Libraries>ClassPath>Tomcat9 설정으로 해결
// WebServlet은 @Controller + @RequestMapping이고, HttpServlet을 상속 받아야하며, service 메소드를 만들어야함
...
public class MyDispatcherServlet extends HttpServlet { 
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			Class clazz = Class.forName("com.jaehee.ch2.YoiilTellerMVC");
			Object obj = clazz.newInstance();
			
			// 1. main 메소드의 정보를 얻는다.
			Method main = clazz.getDeclaredMethod("main", int.class, int.class, int.class, Model.class);
			
			// 2. main 메소드의 매개변수 목록(.paramArr)을 읽어서 메소드의 호출에 사용할 인자 목록(argArr)을 만든다.
	...
	private Object convertTo(Object value, Class type) {
		if(type==null || value==null || type.isInstance(value)) // 타입이 같으면 그래도 반환
			return value;
